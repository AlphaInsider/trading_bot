...
class Binance {
  ...
  async verify() {
    ...
    .then((data) => {
      ...
      if(!data.enableSpotAndMarginTrading && !data.enableMargin && !data.enablePortfolioMarginTrading) return 'invalid';
      return 'valid';
    })
    //invalid
    .catch((error) => {
      //unable to connect
      if(!error.response.data.msg) throw new Error('Unable to connect.');
      return 'invalid';
    })
    //offline
    .catch(() => {
  ...
  async getStocks(params = {}) {
    ...
    if(newStocks.length > 0) {
      ...
      let newBrokerStocks = await newStocks.reduce((chain, curr) => chain.then(async (prev) => {
        //get stock
        let stock = _.find(binanceAssets, {symbol: curr});
  ...
  async getAccountDetails() {
    //get account details, margin details, and btc price
    let [accountDetails, marginDetails, btcInfo] = await Promise.all([
      this._request({
        type: 'get',
        auth: true,
        url: '/api/v3/account'
    ...
    let positions = marginDetails.userAssets.filter(asset => math.evaluate('a != 0', {a: asset.netAsset}) && asset.asset !== 'USDT');
    
    //get margin type
    let marginType = (marginDetails.borrowEnabled ? 'portfolio' : 'cash');
    
    //get portfolio value
    let portfolioValue = math.evaluate('bignumber(a) * bignumber(b)', {a: marginDetails.totalNetAssetOfBtc, b: btcInfo.price}).toString();
    ...
    positions = positions.map((position) => {
      return {
        id: position.asset+'USDT',
        amount: math.evaluate('bignumber(a)', {a: position.netAsset}).toString()
      };
    });
  ...
  async newOrder(params = {}) {
    ...
    //skip if closing and balance is too small
    .catch((error) => {
      //if close and filter error, skip
    ...
    await this._wait(async (time) => {
      ...
      return _.some(openOrders, {orderId: order.orderId});
    }, 1000);
  ...
  async _request(params = {}) {
    ...
    if(params.auth) {
      ...
      let query = (params.query ? new URLSearchParams(params.query).toString() : '');
      let queryString = (params.query ? `${query}&timestamp=${timestamp}` : `timestamp=${timestamp}`);
      
      //create an HMAC SHA256 signature from querystring
      let hmac = crypto.createHmac('sha256', this.binanceSecret);
