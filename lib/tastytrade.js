...
class TastyTrade {
  ...
  async verify() {
    ...
    //invalid
    .catch((error) => {
      if(!error.response.data.error) throw new Error('Unable to connect.');
      return 'invalid';
    })
    //offline
    .catch(() => {
  ...
  async getStocks(params = {}) {
    ...
    if(newStocks.length > 0) {
      ...
      let newBrokerStocks = await newStocks.reduce((chain, curr) => chain.then(async (prev) => {
        ...
        if(!stock.active || stock['instrument-type'] === 'Cryptocurrency') throw new Error('Security is not tradable.');
        //filter data
        let info = {
          id: stock.symbol,
          symbol: stock.symbol,
  ...
  async getAccountDetails() {
    ...
    if(_.find(positions, {'instrument-type': 'Cryptocurrency'})) throw new Error('Unable to trade cryptocurrency positions.');
  ...
  async newOrder(params = {}) {
    ...
    let orders = await Promise.all([wholeShares, fractionalShares].map(async (amount) => {
      ...
      if(math.evaluate('bignumber(a) == 0', {a: amount})) return;
      
      //skip if sell_short and fractional
      if(params.type === 'sell_short' && math.evaluate('a < 1', {a: amount})) return;
