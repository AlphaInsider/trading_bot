...
class Bot extends EventEmitter {
  //DONE: constructor <{alphainsider}> <{broker}> <strategy_id> --multiplier-- --buffer_amount-- --rebalance_on_start-- --close_on_stop--
  constructor(params) {
    //init event emitter
    super();
    ...
    this.status = {
      ...
      rebalancing: false,
      closing: false,
      schedule_rebalance: false,
      schedule_close: false
    }
    
    this.repeatRebalance = false;
    
    this.cron = new Cron('30 0,30 * * * *', () => {
  ...
  async start(params = {}) {
    ...
    this.AlphaInsider.on('reconnecting', async () => {
      this.emit('warning', {type: 'reconnecting', info: {}, message: 'Connection closed, reconnecting.'});
    });
    
    //on error, handle error
    this.AlphaInsider.on('error', async (error) => {
      this.emit('error', {type: 'websocket_error', info: {error}, message: 'Websocket error, stopping bot.'});
      await this.stop();
    });
  ...
  async destroy() {
    ...
    if(this.AlphaInsider && this.Broker) await this.stop({close: false});
  ...
  //DONE: getStatus
  async getStatus() {
    return (this.status.closing || this.status.schedule_close) ? 'closing' : (this.status.rebalancing || this.status.schedule_rebalance) ? 'rebalancing' : (this.status.running) ? 'on' : 'off';
  }
  ...
  //DONE: rebalance
  async rebalance() {
    //skip if rebalance is scheduled
    if(this.status.schedule_rebalance) return;
    ...
    let recursiveRebalance = async () => {
      ...
      //emit rebalance
      this.emit('info', {type: 'rebalance', info: {orders}, message: 'Successfully rebalanced.'});
    ...
    .catch(async (error) => {
      ...
      await this._updateStatus({schedule_rebalance: true});
      throw error;
    })
  ...
  async closeAllPositions() {
    ...
    //success, emit close
    .then((data) => {
      this.emit('info', {type: 'close', info: {orders: data}, message: 'Successfully closed positions.'});
    })
    
    //error, cancel all open orders and schedule close
    .catch(async (error) => {
      await this.Broker.cancelAllOpenOrders().catch((error) => {});
      await this._updateStatus({schedule_close: true});
      throw error;
    })
  ...
  //DONE: _updateStatus --running-- --rebalancing-- --closing-- --schedule_rebalance-- --schedule_close--
  async _updateStatus(params) {
    ...
    j.assert(params, j.object({
      ...
      schedule_rebalance: j.boolean().optional(),
      schedule_close: j.boolean().optional()
    }).required());
  ...
  async _cronSchedule() {
    ...
    if(this.status.schedule_rebalance || this.status.schedule_close) {
      ...
      //close
      if(this.status.schedule_close) {
        //close all positions
        await this.closeAllPositions()
        //error, emit error and stop
        .catch(async (error) => {
          this.emit('error', {type: 'close_error', info: {error}, message: 'Failed to close all positions.'});
          await this.stop({close: false});
        });
      }
      ...
      //rebalance
        ...
        //error, emit error and stop
        .catch(async (error) => {
          this.emit('error', {type: 'rebalance_error', info: {error}, message: 'Failed to rebalance.'});
          await this.stop();
        });
      }
