...
class Bot extends EventEmitter {
  ...
  async rebalance() {
    ...
    let recursiveRebalance = async () => {
      ...
      //emit rebalance
      this.emit('info', {info: {type: 'rebalance', data: orders}, message: 'Successfully rebalanced.'});
  ...
  async _cronSchedule() {
    ...
    if(this.status.scheduled_rebalance || this.status.scheduled_close) {
      ...
      if(this.status.scheduled_close) {
        ...
        .catch(async (error) => {
          this.emit('error', {info: {type: 'close_error', error}, message: 'Failed to close all positions.'});
          await this.stop({close: false});
        });
      }
      ...
      //rebalance
        ...
        //error, emit error and stop
        .catch(async (error) => {
          this.emit('error', {info: {type: 'rebalance_error', error}, message: 'Failed to rebalance.'});
          await this.stop();
        });
      }
