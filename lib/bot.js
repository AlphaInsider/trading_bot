...
class Bot extends EventEmitter {
  ...
  async stop(params = {}) {
    ...
    //check if bot is already closing
    let isClosing = this.status.closing || this.status.scheduled_close;
    ...
    //close on stop
    if(params.close === true || (params.close === undefined && !isClosing && this.closeOnStop)) {
  ...
  async rebalance() {
    ...
    if(this.status.scheduled_rebalance) return;
    ...
    .catch(async (error) => {
      ...
      await this._updateStatus({scheduled_rebalance: true});
      throw error;
    })
  ...
  async closeAllPositions() {
    ...
    .catch(async (error) => {
      await this.Broker.cancelAllOpenOrders().catch((error) => {});
      await this._updateStatus({scheduled_close: true});
      throw error;
    })
