...
class Bot extends EventEmitter {
  ...
  async verify() {
    ...
    .then(async () => {
      ...
      //Broker valid and AlphaInsider valid
        ...
        if(brokerDetails.live && brokerDetails.allocation_type === 'stock' && accountSubscription.type !== 'premium') throw {type: 'error', message: 'Must have a premium account to live trade.'};

        //error if invalid account tier for live crypto trading
        if(brokerDetails.live && brokerDetails.allocation_type === 'cryptocurrency' && !['pro', 'premium'].includes(accountSubscription.type)) throw {type: 'error', message: 'Must have a pro or premium account to live trade.'};
        
        //error if not a stock strategy
        if(brokerDetails.allocation_type === 'stock' && strategyDetails.type !== 'stock') throw {type: 'error', message: 'Strategy being followed must be stock based.'};
        
        //error if not a crypto strategy
        if(brokerDetails.allocation_type === 'cryptocurrency' && strategyDetails.type !== 'cryptocurrency') throw {type: 'error', message: 'Strategy being followed must be cryptocurrency based.'};
  ...
  async calculateNewOrders() {
    ...
    //error if not a stock strategy
    if(brokerDetails.allocation_type === 'stock' && strategyDetails.type !== 'stock') throw new Error('Strategy being followed must be stock based.');
    
    //error if not a crypto strategy
    if(brokerDetails.allocation_type === 'cryptocurrency' && strategyDetails.type !== 'cryptocurrency') throw new Error('Strategy being followed must be cryptocurrency based.');
    ...
    if(math.evaluate('a < b', {a: brokerDetails.value, b: (brokerDetails.allocation_type === 'stock' ? 25000 : 100)})) throw new Error('Account must be above the minimum balance.');
  ...
  async rebalance() {
    ...
    if(math.evaluate('a < b', {a: brokerDetails.value, b: (brokerDetails.allocation_type === 'stock' ? 25000 : 100)})) {
    ...
    let recursiveRebalance = async () => {
      ...
      if(brokerDetails.allocation_type === 'stock' && exchangeStatus.nyse !== 'open') throw new Error('Exchange is closed.');
