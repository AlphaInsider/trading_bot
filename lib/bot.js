...
class Bot extends EventEmitter {
  ...
  async verify() {
    ...
    .then(async () => {
      ...
      //Broker valid and AlphaInsider valid
        ...
        .catch((error) => {
          throw {type: 'error', message: 'Followed strategy no longer available.'};
        });
        ...
        .catch((error) => {
          throw {type: 'error', message: 'Failed to connect to AlphaInsider or broker.'};
        });
        
        //error if invalid account tier for live trading
        if(this.Broker.live && accountSubscription.type !== 'premium') throw {type: 'error', message: 'Account must be premium to live trade.'};
        
        //error if not a stock strategy
        if(strategyDetails.type !== 'stock') throw {type: 'error', message: 'Strategy being followed must be stock based.'};
    ...
    .catch(async (error) => {
      //warning
      if(!(error instanceof Error) && error.type === 'warning') {
        //emit warning
        this.emit('warning', {message: error.message});
        //return
        return 'offline';
      }
      ...
      //error
        ...
        //emit error
        if(!(error instanceof Error)) this.emit('error', {message: error.message});
        //stop bot
        if(brokerStatus === 'valid') await this.stop();
        else await this.stop({close: false});
        //return
        return 'invalid';
      }
    });
  ...
  async start(params = {}) {
    ...
    this.AlphaInsider.on('message', (message) => {
      this.rebalance().catch((error) => {});
    });
  ...
  async closeAllPositions() {
    ...
    .then(async () => {
      ...
      //reset running and scheduled_close
      await this._updateStatus({running: false, scheduled_close: false});
    })
  ...
  async _scheduledActions() {
    ...
    if(this.status.scheduled_rebalance || this.status.scheduled_close) {
      ...
      if(this.status.scheduled_close) {
        ...
        //error, emit error and throw error to retry
        .catch((error) => {
