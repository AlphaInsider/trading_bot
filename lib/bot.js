...
class Bot extends EventEmitter {
  ...
  async calculateNewOrders() {
    ...
    //error if not a stock strategy
    if(this.Broker.allocationType === 'stock' && strategyDetails.type !== 'stock') throw new Error('Must be a stock broker.');

    //error if not a crypto strategy
    if(this.Broker.allocationType === 'cryptocurrency' && strategyDetails.type !== 'cryptocurrency') throw new Error('Must be a cryptocurrency broker.');
    ...
    if(this.Broker.allocationType === 'stock' && math.evaluate('a < 25000', {a: brokerDetails.value})) throw new Error('Account must maintain a value of at least $25,000.');
  ...
  async rebalance() {
    ...
    //error and stop if account is less than 25k
    let brokerDetails = await this.Broker.getAccountDetails();
    if(this.Broker.allocationType === 'stock' && math.evaluate('a < 25000', {a: brokerDetails.value})) {
      this.emit('error', {info: {type: 'rebalance'}, message: 'Account must maintain a minimum balance of $25,000.'});
      await this.stop();
      throw new Error('Account must maintain a minimum balance of $25,000.');
    }
