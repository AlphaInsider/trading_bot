...
class Bot extends EventEmitter {
  ...
  async verify() {
    ...
    .then(async () => {
      ...
      //Broker valid and AlphaInsider valid
        ...
        if(this.Broker.live && this.Broker.assetClass === 'stock' && accountSubscription.type !== 'premium') throw {type: 'error', message: 'Must have a premium account to live trade.'};

        //error if invalid account tier for live crypto trading
        if(this.Broker.live && this.Broker.assetClass === 'cryptocurrency' && !['pro', 'premium'].includes(accountSubscription.type)) throw {type: 'error', message: 'Must have a pro or premium account to live trade.'};
        
        //error if not a stock strategy
        if(this.Broker.assetClass === 'stock' && strategyDetails.type !== 'stock') throw {type: 'error', message: 'Strategy being followed must be stock based.'};
        
        //error if not a crypto strategy
        if(this.Broker.assetClass === 'cryptocurrency' && strategyDetails.type !== 'cryptocurrency') throw {type: 'error', message: 'Strategy being followed must be cryptocurrency based.'};
  ...
  async calculateNewOrders() {
    ...
    //error if not a stock strategy
    if(this.Broker.assetClass === 'stock' && strategyDetails.type !== 'stock') throw new Error('Strategy being followed must be stock based.');
    
    //error if not a crypto strategy
    if(this.Broker.assetClass === 'cryptocurrency' && strategyDetails.type !== 'cryptocurrency') throw new Error('Strategy being followed must be cryptocurrency based.');
    ...
    if(math.evaluate('a < b', {a: brokerDetails.value, b: (this.Broker.assetClass === 'stock' ? 25000 : 100)})) throw new Error('Account must be above the minimum balance.');
  ...
  async rebalance() {
    ...
    if(math.evaluate('a < b', {a: brokerDetails.value, b: (this.Broker.assetClass === 'stock' ? 25000 : 100)})) {
