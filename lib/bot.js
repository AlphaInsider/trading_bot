...
class Bot extends EventEmitter {
  ...
  constructor(params = {}) {
    ...
    try {
      ...
      else if(params.broker.type === 'bitfinex') {
        this.Broker = new lib.Bitfinex({
          bitfinex_key: params.broker.bitfinex_key,
          bitfinex_secret: params.broker.bitfinex_secret
        });
      }
  ...
  async verify() {
    ...
    .then(async () => {
      ...
      //Broker valid and AlphaInsider valid
        ...
        if(brokerDetails.live && brokerDetails.asset_class === 'stock' && accountSubscription.type !== 'premium') throw {type: 'error', message: 'Must have a premium account to live trade.'};

        //error if invalid account tier for live crypto trading
        if(brokerDetails.live && brokerDetails.asset_class === 'cryptocurrency' && !['pro', 'premium'].includes(accountSubscription.type)) throw {type: 'error', message: 'Must have a pro or premium account to live trade.'};
        
        //error if not a stock strategy
        if(brokerDetails.asset_class === 'stock' && strategyDetails.type !== 'stock') throw {type: 'error', message: 'Strategy being followed must be stock based.'};
        
        //error if not a crypto strategy
        if(brokerDetails.asset_class === 'cryptocurrency' && strategyDetails.type !== 'cryptocurrency') throw {type: 'error', message: 'Strategy being followed must be cryptocurrency based.'};
  ...
  async calculateNewOrders() {
    ...
    //error if not a stock strategy
    if(brokerDetails.asset_class === 'stock' && strategyDetails.type !== 'stock') throw new Error('Strategy being followed must be stock based.');
    
    //error if not a crypto strategy
    if(brokerDetails.asset_class === 'cryptocurrency' && strategyDetails.type !== 'cryptocurrency') throw new Error('Strategy being followed must be cryptocurrency based.');
    ...
    if(math.evaluate('a < b', {a: brokerDetails.value, b: (brokerDetails.asset_class === 'stock' ? 25000 : 100)})) throw new Error('Account must be above the minimum balance.');
  ...
  async rebalance() {
    ...
    if(math.evaluate('a < b', {a: brokerDetails.value, b: (brokerDetails.asset_class === 'stock' ? 25000 : 100)})) {
    ...
    let recursiveRebalance = async () => {
      ...
      if(brokerDetails.asset_class === 'stock' && exchangeStatus.nyse !== 'open') throw new Error('Exchange is closed.');
  ...
  async closeAllPositions() {
    ...
    .then(async () => {
      ...
      let exchangeStatus = await this.AlphaInsider.getExchangeStatus();
      if(this.Broker.assetClass === 'stock' && exchangeStatus.nyse !== 'open') throw new Error('Exchange is closed.');
