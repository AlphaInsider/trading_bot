...
class Server {
  ...
  async _refreshSettings() {
    ...
    try {
      ...
      this.Bot.on('status', async ({status, prevStatus}) => {
        await this.knex('bot').update({status: status, updated_at: moment().toISOString()});
        this.wsMessage({channel: 'wsStatus', message: status});
        if(status === 'on' && prevStatus === 'off') this._newActivity({type: 'info', message: 'Bot started.'});
        if(status === 'off') this._newActivity({type: 'info', message: 'Bot stopped.'});
      });
      this.Bot.on('info', (data) => this._newActivity({...data, type: 'info'}));
      this.Bot.on('warning', (data) => this._newActivity({...data, type: 'warning'}));
      this.Bot.on('error', (data) => this._newActivity({...data, type: 'error'}));
    }
    catch(error) {}
