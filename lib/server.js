...
class Server {
  ...
  async updateBrokerAlpaca(params = {}) {
    ...
    let transaction = async (trx) => {
      //clear strategy allocation
      if(this.Broker && this.Broker.assetClass !== 'stock') {
  ...
  async updateBrokerBinance(params = {}) {
    //init
    await this.ready;
    ...
    let binance = new lib.Binance({
      binance_key: params.binance_key,
      binance_secret: params.binance_secret
    });
    let brokerDetails = await binance.getAccountDetails();
    await binance.destroy();
    
    //verify user permissions
    if(brokerDetails.live) {
      let accountSubscription = await this.AlphaInsider.getAccountSubscription();
      if(!['pro', 'premium'].includes(accountSubscription.type)) throw new Error('Invalid permissions.');
    }
    
    //update broker details
    let transaction = async (trx) => {
      //clear strategy allocation
      if(this.Broker && this.Broker.assetClass !== 'cryptocurrency') {
        await this._clearAllocation(trx);
      }
  ...
  //DONE: updateBrokerBitfinex <bitfinex_key> <bitfinex_secret>
  async updateBrokerBitfinex(params = {}) {
    //init
    await this.ready;
    ...
    let bitfinex = new lib.Bitfinex({
      bitfinex_key: params.bitfinex_key,
      bitfinex_secret: params.bitfinex_secret
    });
    let brokerDetails = await bitfinex.getAccountDetails();
    ...
    let transaction = async (trx) => {
      //clear strategy allocation
      if(this.Broker && this.Broker.allocationType !== 'cryptocurrency') {
        await this._clearAllocation(trx);
      }
