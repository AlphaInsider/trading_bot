...
class AlphaInsider extends EventEmitter {
  ...
  //DONE: getStocks --[alpha_stock_ids]--
  async getStocks(params = {}) {
    ...
    //filter new stocks to get
    let newStockIds = _.difference(params.alpha_stock_ids, Object.keys(this.alphaStocks));
    ...
    newStocks = newStocks.map((item) => ({
      ...item,
      expires_at: moment().add(60*60*4, 'seconds').toISOString()
    }));
    ...
    //return
    return params.alpha_stock_ids.map((id) => this.alphaStocks[id]);
  }
  
  //DONE: getStockPrices <[alpha_stock_ids]>
  ...
  //DONE: getStrategyDetails <strategy_id>
  ...
  //DONE: _getPositions <strategy_id>
