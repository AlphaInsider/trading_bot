...
class Alpaca {
  ...
  async verify() {
    ...
    //invalid
    .catch((error) => {
      if(error.code === 'ENOTFOUND' || error.code === 'ECONNABORTED' || !error.response.data.message) throw new Error('Unable to connect.');
      return 'invalid';
    })
    //offline
    .catch(() => {
  ...
  async getAccountDetails() {
    ...
    //format positions
    positions = _.chain(positions).map((position) => {
      //skip if not stock asset
      if(position.asset_class === 'crypto') return;
      return {
        id: position.symbol,
        amount: position.qty
      };
  ...
  async newOrder(params = {}) {
    ...
    let order = await Promise.resolve().then(async () => {
      ...
      //round amount
      let amount = math.evaluate('floor(bignumber(a) / bignumber(b)) * bignumber(b)', {a: params.amount, b: brokerStock.step_size}).toString();
